...
class GraphDataset(InMemoryDataset):
    def __init__(self, root='/tmp', transform=None, pre_transform=None, graphs_dict=None, dttype=None, drug_smiles=None, target_smiles=None):
        super(GraphDataset, self).__init__(root, transform, pre_transform)
        self.dttype = dttype
        self.process(graphs_dict, drug_smiles, target_smiles)
    ...
    def process(self, graphs_dict, drug_smiles, target_smiles):
        data_list = []
        for key in graphs_dict:
            size, features, edge_index = graphs_dict[key]
            if drug_smiles != None:
                GCNData = DATA.Data(x=torch.Tensor(features), edge_index=torch.LongTensor(edge_index).transpose(1, 0), drug_feature=drug_smiles[key])
            else:
                GCNData = DATA.Data(x=torch.Tensor(features), edge_index=torch.LongTensor(edge_index).transpose(1, 0), target_feature=target_smiles[key])
